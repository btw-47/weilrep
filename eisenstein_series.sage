...
def iard(a,r,d,p,t):
    ...
    if r % 2 == 0:
        ...
        if a % p == 0:
            ...
            iard = 1 - 1/p - (p ** (-r)) * (1 - t/p) + (p ** ((-r) // 2)) * kronecker_symbol(-a * d * (-1)^(r // 2), p) * (t - 1)
    return iard
...
def hat_hq_v(a,u,Q0,t): #Helper function labeled H_1(a,u,Q0) in section 12 of [W]
    ...
    a_vals = [x.valuation(2) for x in a]
    if Q0[0]:
        return ig_v_from_valuations(a_vals, u, 0,t) - (2 ** (-r)) * ig_v_from_valuations(a_vals, u, 1,t)
    else:
        return (1 - (2 ** (-r))) * ig_v_from_valuations(a_vals,u,1,t)
...
def tilde_hq_v(a,u,Q0,t): #Helper function labeled H_2(a,u,Q0) in section 12 of [W]
    ...
    if len(Q0[0]) == 0:
        ...
        b = Q0[0][0]
        b = vector([b]*len(a))
        return (1 - 2*eps_two_r)*ig_v_from_valuations(a_vals,u,0,t) - (2 ** (-r)) * ig_v_from_valuations(a_vals,u,2,t) + eps_two_r * (ig_v_from_valuations(a_vals,u,2,t) + ig_v(a+b,u,2,t))
    else:
        b = Q0[0][0]
        c = Q0[0][1]
        if (b+c)%4:
            b = vector([b]*len(a))
            return (1 - 2*eps_two_r)*ig_v_from_valuations(a_vals,u,0,t) + eps_two_r * ig_v_from_valuations(a_vals,u,1,t) + eps_two_r * ig_v(a+b,u,2,t) - (2 ** (-r)) * ig_v_from_valuations(a_vals,u,2,t)
        else:
            return ig_v_from_valuations(a_vals,u,0,t) - eps_two_r * ig_v_from_valuations(a_vals,u,1,t) + (eps_two_r - (2 ** (-r))) * ig_v_from_valuations(a_vals,u,2,t)
...
def tilde_hq_diff_v(a,u,Q0,t): #Helper function labeled H_3(a,u,Q0) in section 12 of [W]
    ...
    if len(Q0[0]) == 1:
        b = Q0[0][0]
        b = vector([b]*len(a))
        ab_vals = [x.valuation(2) for x in a+b]
...
def tilde_hq(a,u,Q0,t): #H2
    ...
    if len(Q0[0]) == 0:
        ...
        b = Q0[0][0]
        return (1 - 2 * two_r)*ig(a,u,0,t) - (2 ** (-r)) * ig(a,u,2,t) + two_r * (ig(a,u,2,t) + ig(a+b,u,2,t))
    else:
        b, c = Q0[0]
        if (b+c)%4:
            return (1 - 2 * two_r)*ig(a,u,0,t) + two_r * (ig(a,u,1,t) + ig(a+b,u,2,t)) - (2 ** (-r)) * ig(a,u,2,t)
        else:
            return ig(a,u,0,t) - two_r * ig(a,u,1,t) + (two_r - (2 ** (-r))) * ig(a,u,2,t)
...
def twoadic_igusa_zetas(Q,L,c_list, t): #compute several igusa zeta functions at the same time\
    ...
    vc_s = tuple(n.valuation(2) for n in c)
    try:
    ...
    if vb == Infinity:
        ...
        for j, cj in enumerate(c):
            ...
            if cj:
                ...
                u.append(((t ** (w-1)) * iaqqq(0, Infinity, Qs[w-1], Qs[w], zero_form ,t) / q_powers[w-1] + (t ** w) * iaqqq(0, Infinity, Qs[w], Qs[w-1], zero_form,t) / q_powers[w]) / (1 - t * t * (2 ** (-big_r))))
        return Z + vector(u)
    else:
...
def weight_one_zero_val(g,S):
    r"""
    Compute the constant term in the Eisenstein series of weight 1.

    INPUT:
    - ``g`` -- a vector for which g*S is integral
    - ``S`` -- a Gram matrix (symmetric integral matrix with even diagonal and nonzero determinant)

    OUTPUT: a correction term to be subtracted from the constant term of the Eisenstein series of weight 1

    EXAMPLES::

        sage: S = matrix([[4,0],[0,2]])
        sage: weight_one_zero_val(vector([0,0]),S)
        1

        sage: S = matrix([[-4,1],[1,-2]])
        sage: weight_one_zero_val(vector([0,0]),S)
        -1

        sage: S = matrix([[-2,0,0,0],[0,-2,0,0],[0,0,-2,0],[0,0,0,2]])
        sage: weight_one_zero_val(vector([0,0,0,0]),S)
        -1/2

    """
    if S.nrows() == 2: #kluge. should eventually be fixed
        S = block_diagonal_matrix([S, matrix([[0,1],[1,0]])])
        g = vector([g[0],g[1],0,0])
    Q = QuadraticForm(S)
    L = g*S
    if frac(g*L/2):
        return 0
    L = L + L
    D = abs(S.determinant())
    s = pi*(-1)^((2 - Q.signature())  /4) / sqrt(D) #this needs to be fixed!! we are probably doing something weird like using the symbolic ring!!
    s /= quadratic_L_function__correct(1, -D)
    for p in D.prime_factors():
        j = D.valuation(p)
        s *= L_values(L, [0], S, p, 1)[0]
        try:
            s *= L_values(L, [0], S, p, 1)[0]
        except ZeroDivisionError:
            ...
            p_power = p ** (d / 2)
            if p == 2:
                ig_z = (1 - twoadic_igusa_zetas(Q, L, [0], t)[0]) * (1 - t/2) * (1 - t / p_power) / (1 - t)
            else:
                ig_z = (1 - t * igusa_zetas(Q,L,[0],p)[0]) * (1 - t/p) * (1 - t / p_power) / (1 - t)
            s *= ig_z(p^(d/2))
            D //= p^j
    return QQ(s * quadratic_L_function__correct(0, -D))