...
class OrthogonalModularFormsLorentzian(object):
    ...
    def eisenstein_series(self, k, prec):
        ...
        """
        w = self.__weilrep
        nrows = self.__gram_matrix.nrows()
        try:
...
class OrthogonalModularFormLorentzian:
    ...
    def __repr__(self):
        ...
        try:
            ...
            _m = lambda s: sub(r'\^-?\d+|(?<!O\(|\, )(\_\d+|q|s)(?!\^)', m, s)
            if self.__qexp_representation == 'PD+II':
                ...
                try:
                    ...
                    try:
                        ...
                        rs.<s> = LaurentSeriesRing(self.base_ring())
                        rq.<q> = LaurentSeriesRing(rs)
                        qs = False
                        try:
    ...
    def __mul__(self, other):
        ...
        if isinstance(other, OrthogonalModularFormLorentzian):
            ...
            if self_scale != 1 or other_scale != 1:
                ...
                X1 = self.rescale(new_scale // self_scale)
                X2 = other.rescale(new_scale // other_scale)
                return OrthogonalModularFormLorentzian(self.__weight, self.__gram_matrix, X1.true_fourier_expansion() * X2.true_fourier_expansion(), scale = new_scale, weylvec = self.weyl_vector() + other.weyl_vector(), qexp_representation = self.__qexp_representation)
...
class WeilRepModularFormLorentzian(WeilRepModularForm):
    ...
    def theta_lift(self, prec = None):
        ...
        s_0 = s_0[:-1,:-1]
        s_0inv = s_0.inverse()
        new_prec = ceil(prec^2 * scale / (-4 * b_norm) + prec)
        if nrows >= 3:
                ...
                try:
                    ...
                    if nrows >= 3:
                        if nrows >= 4:
                            ...
                            m *= (rb.0)**v[1]
                    lift += c * sum([n^(k - 1) * (m^n + eps * m^(-n)) * t^(n * j) for n in range(1, prec_j)])
                except KeyError:
        ...
        for j in range(1, prec):
            ...
            try:
                c = coeffs[tuple([frac(y) for y in z] + [-norm_z] )]
                lift += c * sum([n^(k - 1) * t^(n * j) for n in range(1, prec//j + 1)])
            except KeyError:
                if -norm_z >= self.precision():
                    prec= j
                    break
        ...
        if nrows > 1:
            ...
            if N == 1 or N.is_prime():
                ...
                if val < 0:
                    ...
                    e = w.dual().eisenstein_series(3/2, max(1, 1 - ceil(val)), allow_small_weight = True, components = (_ds, indices)).fourier_expansion()
            ...
            if s:
                if N % 2:
                    if -val < 10:
                        X = WeilRepModularForm(-1/2, matrix([[4]]), [(vector([0]), 0, 4 + 16*q + 56*q^2 + 160*q^3 + 400*q^4 + 928*q^5 + 2016*q^6 + 4160*q^7 + 8248*q^8 + 15792*q^9 + O(q^10)), ((1/4), -1/8, 1 - q + q^2 - 2*q^3 + 3*q^4 - 4*q^5 + 5*q^6 - 7*q^7 + 10*q^8 - 13*q^9 + 16*q^10 + O(q^11)), ((1/2), -1/2, -8*q - 32*q^2 - 96*q^3 - 256*q^4 - 616*q^5 - 1376*q^6 - 2912*q^7 - 5888*q^8 - 11456*q^9 - 21600*q^10 + O(q^11)), ((3/4), -1/8, 1 - q + q^2 - 2*q^3 + 3*q^4 - 4*q^5 + 5*q^6 - 7*q^7 + 10*q^8 - 13*q^9 + 16*q^10 + O(q^11))])
                    else:
                        X = WeilRep(matrix([[4]])).nearly_holomorphic_modular_forms_basis(-1/2, 1, ceil(-val))[0]
                    p_0 = 2
                elif N % 3:
                    if -val < 10:
                        X = WeilRepModularForm(-1/2, matrix([[6]]), [(vector([0]), 0, 2 + 4*q + 12*q^2 + 24*q^3 + 52*q^4 + 96*q^5 + 180*q^6 + 312*q^7 + 540*q^8 + 892*q^9 + O(q^10)), ((1/6), -1/12, 1 + 2*q + 5*q^2 + 12*q^3 + 24*q^4 + 46*q^5 + 85*q^6 + 150*q^7 + 257*q^8 + 430*q^9 + 701*q^10 + O(q^11)), ((1/3), -1/3, -2*q - 4*q^2 - 10*q^3 - 20*q^4 - 40*q^5 - 72*q^6 - 132*q^7 - 224*q^8 - 380*q^9 - 620*q^10 + O(q^11)), ((1/2), -3/4, -2*q - 6*q^2 - 14*q^3 - 30*q^4 - 60*q^5 - 114*q^6 - 206*q^7 - 360*q^8 - 612*q^9 - 1014*q^10 + O(q^11)), ((2/3), -1/3, -2*q - 4*q^2 - 10*q^3 - 20*q^4 - 40*q^5 - 72*q^6 - 132*q^7 - 224*q^8 - 380*q^9 - 620*q^10 + O(q^11)), ((5/6), -1/12, 1 + 2*q + 5*q^2 + 12*q^3 + 24*q^4 + 46*q^5 + 85*q^6 + 150*q^7 + 257*q^8 + 430*q^9 + 701*q^10 + O(q^11))])
                    else:
                        X = WeilRep(matrix([[6]])).nearly_holomorphic_modular_forms_basis(-1/2, 1, ceil(-val))[0]
                    p_0 = 3
                else:
                    sqr = True
            if sqr or not s:
                if -val < 10:
                    X = WeilRepModularForm(-1/2, matrix([[2]]), [(vector([0]), 0, 10 + 108*q + 808*q^2 + 4016*q^3 + 16524*q^4 + 58640*q^5 + 188304*q^6 + 556416*q^7 + 1541096*q^8 + 4038780*q^9 + O(q^10)), ((1/2), -1/4, 1 - 64*q - 513*q^2 - 2752*q^3 - 11775*q^4 - 43200*q^5 - 141826*q^6 - 427264*q^7 - 1201149*q^8 - 3189120*q^9 - 8067588*q^10 + O(q^11))])
                else:
                    X = WeilRep(matrix([[2]])).nearly_holomorphic_modular_forms_basis(-1/2, 1, ceil(-val))[0]
                p_0 = 1
            X = self * X
            m = X.valuation(exact = True)
            ...
            while i < 2:
                jN = j * j * N
                for p in range(jN - j^2 * scale - 1, jN):
                    d = jN - p
                    f = isqrt(d // p_0)
            ...
            return vector([(matrix(v).solve_right(vector(h)) * p_0 / 12)[0]])
    ...
    def borcherds_lift(self, prec = None):
                    ...
                    if c > 0:
                        ...
                        for j in range(2, isqrt(-val / norm_z) + 1):
                            try:
                                ...
                                c_new = coeffs[tuple([frac(j * y) for y in z] + [-j^2 * norm_z] )]
                                p *= (1 - tpoly^j)^c_new
                            except KeyError:
                                pass
        ...
        if nrows >= 3:
            if nrows >= 4:
                ...
                weyl_monomial *= (rb.0)**weyl_vector[1]
