...
class WeilRep(object):
    ...
    def eisenstein_series(self, k, prec, allow_small_weight = False, components = None):
        ...
        if not k_is_list:
            ...
            if k <= 2 and not allow_small_weight:
                d = self.discriminant()
                if k < 2 or not ((d%4 and self.discriminant().is_squarefree()) or (self.discriminant()//4).is_squarefree()):
    ...
    def theta_series(self, prec, P = None, test_P = True):
        ...
        try:
            ...
            for v in vs_list:
                ...
                list_g = map(frac, g)#[frac(x) for x in g]
                g = tuple(list_g)
                j1 = _ds_dict[g]
    ...
    def cusp_forms_dimension(self, weight, eta_twist = 0, force_Riemann_Roch = False, do_not_round = False):
        ...
        if weight <= 0 or symm is None:
            return 0
        elif weight > 2 or (weight == 2 and (eta_twist or not self.discriminant().is_squarefree())) or force_Riemann_Roch:
            eps = 1 if symm else -1
            ...
            try:
                ...
                gauss_sum_1 = exp(pi_i * sig / 4) * sqrt_A #milgram formula. unfortunately we are going to iterate over the discriminant group anyway
